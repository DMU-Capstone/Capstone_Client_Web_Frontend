...
import { apiPost, errorUtils, API_BASE_URL } from "../../../utils/api"; // ✅ apiPost 사용 (없다면 fetch로 교체)
...
const QueueCreate: React.FC = () => {
  ...
  // "HH:mm" -> 오늘 날짜 기준 ISO (Z)
  const timeToTodayISO = (hhmm: string) => {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      // 스웨거 예시 구조에 맞춘 request 바디
      const requestPayload = {
        hostName: form.hostName,
        maxPeople: Number(form.maxPeople) || 0,
        hostManagerName: form.ownerName,
        hostPhoneNumber: form.phone,
        latitude: 0.0,      // TODO: 위치 연동 시 실제 값으로
        longitude: 0.0,     // TODO: 위치 연동 시 실제 값으로
        keyword: keywords.join(", "), // 서버가 단일 string을 받는 명세라면 콤마 연결
        description: form.about,
        startTime: timeToTodayISO(form.openTime),
        endTime: timeToTodayISO(form.closeTime),
      };

      // 멀티파트 폼: request(JSON) + hostImages(File[])
      const fd = new FormData();
      fd.append(
        "request",
        new Blob([JSON.stringify(requestPayload)], { type: "application/json" })
      );
      photos.forEach((p) => fd.append("hostImages", p.file));

      // 엔드포인트: POST /host
      const res = await apiPost<FormData | unknown>(
        "/host",
        fd,
        {
          // apiPost가 axios 기반이면 headers 생략(브라우저가 자동 설정)
          // fetch 기반이라면 'Content-Type'를 절대 수동 지정하지 말 것 (경계)
        }
      );

      if ((res as any)?.success === false) {
        throw new Error((res as any)?.message || "호스트 생성에 실패했습니다.");
      }
      ...
      // 실패 메시지
      const msg = errorUtils?.getErrorMessage
        ? errorUtils.getErrorMessage(err)
        : (err as Error)?.message || "호스트 생성 중 오류가 발생했습니다.";
      alert(msg);
      console.error(err);
    } finally {
      setSubmitting(false);
      ...
      <div className="w-full max-w-[420px] px-4 py-4">
        ...
        <form onSubmit={onSubmit} className="mt-4 space-y-5">
          ...
          <button
            type="submit"
            disabled={submitting}
            className="w-full h-11 rounded-full bg-indigo-600 text-white font-semibold hover:bg-indigo-700 active:scale-[0.99] disabled:opacity-60"
          >
            {submitting ? "생성 중..." : "호스트 생성하기"}
          </button>
        </form>
      </div>
    </div>
