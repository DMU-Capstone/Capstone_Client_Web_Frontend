...
const EndQueueModal: React.FC<RegisterQueueModalProps> = ({
  open,
  onClose,
}) => {
  ...
  const fetchEndQueue = async () => {
    try {
      ...
      const ended = allQueues.filter(
        (q) => new Date(q.endTime).getTime() <= now
      );
  ...
  return (
    <Modal open={open} onClose={onClose}>
      <Box
        className="modalBox"
        sx={{ width: 1000, maxHeight: "90%", overflowY: "auto" }}
      >
        <Box
          display="flex"
